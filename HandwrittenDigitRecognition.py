...
def trainAndPredictMLP(X_train, Y_train, X_test, Y_test):
    ...
    # Exercice 4
    model.save_weights("MLPweights")

    return

def exercice5(X_test, Y_test):
    num_pixels = X_test.shape[1] * X_test.shape[2]

    # make a prediction on the first 5 images
    X_test = X_test.reshape((X_test.shape[0], num_pixels)).astype('float32')
    X_test = X_test[:5]
    X_test = X_test / 255
    Y_test = np_utils.to_categorical(Y_test)
    ...
    model.add(Dense(8, input_dim=784, kernel_initializer='normal',
                    activation='relu'))
    model.add(Dense(10, kernel_initializer='normal',
                    activation='softmax'))

    model.load_weights("MLPweights")
    ...
    for i in predictions:
        print("image {} of predicted class : ".format(compteur) + str(np.argmax(i)))
        print("image {} of actual class    : ".format(compteur) + str(np.argmax(Y_test[compteur])))
        compteur += 1
